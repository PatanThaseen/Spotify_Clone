...
// Get all download icons
const downloadIcons = document.querySelectorAll('.downloadSong');

// Add event listener to each download icon
downloadIcons.forEach(icon => {
    icon.addEventListener('click', function(event) {
        // Prevent the default behavior of the link (opening in the same window)
        event.preventDefault();

        // Get the href attribute of the download link
        const songUrl = icon.getAttribute('href');

        // Create an invisible link element to trigger the download
        const downloadLink = document.createElement('a');
        downloadLink.href = songUrl;
        downloadLink.download = icon.getAttribute('download');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Array to store original song items
    let originalSongs = [];
    ...
    // Save original song items on page load
